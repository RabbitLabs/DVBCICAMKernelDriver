...
static int dvbciusb_command_open(struct inode *inode, struct file *file)
{
...
static int dvbciusb_command_flush(struct file *file, fl_owner_t id)
{
	struct usbdvbci_interface *dev;
	int res;
	bool ongoing_io;
...
static void dvbciusb_command_read_bulk_callback(struct urb *urb)
{
	struct usbdvbci_interface *dev;
...
static int dvbciusb_command_do_read_io(struct usbdvbci_interface *dev, size_t count)
{
	int rv;
	...
	usb_fill_bulk_urb(dev->bulk_in_urb,
			...
			dev->bulk_in_buffer,
		  read_length,
			dvbciusb_command_read_bulk_callback,
			dev);
...
static ssize_t dvbciusb_command_read(struct file *file, char *buffer, size_t count,
			 loff_t *ppos)
{
	struct usbdvbci_interface *dev;
	int rv;
...
retry:
	...
	rv = dvbciusb_command_do_read_io(dev, count);
	if (rv < 0)
		goto exit;
...
static void dvbciusb_command_write_bulk_callback(struct urb *urb)
{
	struct usbdvbci_interface *dev;
...
static ssize_t dvbciusb_command_write(struct file *file, const char *user_buffer,
			  size_t count, loff_t *ppos)
{
	...
	usb_fill_bulk_urb(urb, dev->udev,
			  ...
			  buf, writesize, dvbciusb_command_write_bulk_callback, dev);
...
static const struct file_operations dvbciusb_command_fops = {
	.owner =	THIS_MODULE,
	.read =		dvbciusb_command_read,
	.write =	dvbciusb_command_write,
	.open =		dvbciusb_command_open,
	.release =	dvbciusb_command_release,
	.flush =	dvbciusb_command_flush,
	.llseek =	noop_llseek,
};
 ...
 */
struct usb_class_driver dvbciusb_command_class = {
	.name =		"usbdvbcicmd%d",
	.fops =		&dvbciusb_command_fops,
	.minor_base =	USB_DVBCIUSB_COMMAND_MINOR_BASE,
};
